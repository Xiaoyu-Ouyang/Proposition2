...
A public-key quantum money scheme consists of three algorithms:
\begin{itemize}
    \item $\textsc{KeyGen}(1^\lambda) \to (\mathsf{sk},\mathsf{pk})$: Generates a secret minting key and a public verification key.
    \item $\textsc{Mint}(\mathsf{sk}) \to |\$\rangle$: Prepares a valid quantum banknote.
    \item $\textsc{Verify}(\mathsf{pk},|\psi\rangle) \to \top / \bot$: Publicly verifies whether the state $|\psi\rangle$ is a valid banknote.
\end{itemize}
